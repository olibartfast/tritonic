...
public:
    ...
    void SetServerAddress(const std::string& v) { server_address_ = v; }
    ...
    void SetProtocol(const std::string& v) { protocol_ = v; }
    ...
    void SetModelVersion(const std::string& v) { model_version_ = v; }
    const std::string& GetModelType() const noexcept { return model_type_; }
    void SetModelType(const std::string& v) { model_type_ = v; }
    const std::vector<std::vector<int64_t>>& GetInputSizes() const noexcept { return input_sizes_; }
    void SetInputSizes(const std::vector<std::vector<int64_t>>& v) { input_sizes_ = v; }
    ...
    void SetLabelsFile(const std::string& v) { labels_file_ = v; }
    int GetBatchSize() const noexcept { return batch_size_; }
    void SetBatchSize(int v) { batch_size_ = v; }

    // Processing
    bool GetShowFrame() const noexcept { return show_frame_; }
    void SetShowFrame(bool v) noexcept { show_frame_ = v; }
    bool GetWriteFrame() const noexcept { return write_frame_; }
    void SetWriteFrame(bool v) noexcept { write_frame_ = v; }
    float GetConfidenceThreshold() const noexcept { return confidence_threshold_; }
    void SetConfidenceThreshold(float v) noexcept { confidence_threshold_ = v; }
    float GetNmsThreshold() const noexcept { return nms_threshold_; }
    void SetNmsThreshold(float v) noexcept { nms_threshold_ = v; }

    // Shared memory
    const std::string& GetSharedMemoryType() const noexcept { return shared_memory_type_; }
    void SetSharedMemoryType(const std::string& v) { shared_memory_type_ = v; }
    int GetCudaDeviceId() const noexcept { return cuda_device_id_; }
    void SetCudaDeviceId(int v) noexcept { cuda_device_id_ = v; }
    ...
    void SetLogFile(const std::string& v) { log_file_ = v; }

    // Multimodal and text-generation support
    bool GetEnableMultimodal() const noexcept { return enable_multimodal_; }
    void SetEnableMultimodal(bool v) noexcept { enable_multimodal_ = v; }
    const std::string& GetTextInput() const noexcept { return text_input_; }
    void SetTextInput(const std::string& v) { text_input_ = v; }
    const std::string& GetAudioInput() const noexcept { return audio_input_; }
    void SetAudioInput(const std::string& v) { audio_input_ = v; }
    const std::string& GetTextPrompt() const noexcept { return text_prompt_; }
    void SetTextPrompt(const std::string& v) { text_prompt_ = v; }
    const std::string& GetModalityCombination() const noexcept { return modality_combination_; }
    void SetModalityCombination(const std::string& v) { modality_combination_ = v; }
    float GetTextWeight() const noexcept { return text_weight_; }
    void SetTextWeight(float v) noexcept { text_weight_ = v; }
    float GetImageWeight() const noexcept { return image_weight_; }
    void SetImageWeight(float v) noexcept { image_weight_ = v; }
    float GetAudioWeight() const noexcept { return audio_weight_; }
    void SetAudioWeight(float v) noexcept { audio_weight_ = v; }
    int GetMaxTokens() const noexcept { return max_tokens_; }
    void SetMaxTokens(int v) noexcept { max_tokens_ = v; }
    float GetTemperature() const noexcept { return temperature_; }
    void SetTemperature(float v) noexcept { temperature_ = v; }
    float GetTopP() const noexcept { return top_p_; }
    void SetTopP(float v) noexcept { top_p_ = v; }
    float GetRepetitionPenalty() const noexcept { return repetition_penalty_; }
    void SetRepetitionPenalty(float v) noexcept { repetition_penalty_ = v; }
    const std::string& GetStopWords() const noexcept { return stop_words_; }
    void SetStopWords(const std::string& v) { stop_words_ = v; }

    // Chat (OpenAI-compatible) backend
    const std::string& GetBackend() const noexcept { return backend_; }
    void SetBackend(const std::string& v) { backend_ = v; }
    const std::string& GetApiEndpoint() const noexcept { return api_endpoint_; }
    void SetApiEndpoint(const std::string& v) { api_endpoint_ = v; }
    const std::string& GetApiKeyEnv() const noexcept { return api_key_env_; }
    void SetApiKeyEnv(const std::string& v) { api_key_env_ = v; }
    int GetTargetImageSize() const noexcept { return target_image_size_; }
    void SetTargetImageSize(int v) noexcept { target_image_size_ = v; }
    bool GetInteractive() const noexcept { return interactive_; }
    void SetInteractive(bool v) noexcept { interactive_ = v; }
