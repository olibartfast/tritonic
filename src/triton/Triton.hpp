...
struct SharedMemoryRegion {
    ...
    SharedMemoryRegion()
        : ptr(nullptr),
          size(0),
          fd(-1),
          type(SharedMemoryType::SYSTEM_SHARED_MEMORY),
          device_id(0) {}
    ~SharedMemoryRegion() {
        cleanup();
    }
...
public:
    ...
    void printModelInfo(const TritonModelInfo& model_info) const override {
        auto logger = std::dynamic_pointer_cast<Logger>(LoggerManager::GetLogger("triton"));
        ...
        for (size_t i = 0; i < model_info.input_names.size(); ++i) {
            ...
            for (size_t j = 0; j < model_info.input_shapes[i].size(); ++j) {
                ...
                if (j < model_info.input_shapes[i].size() - 1)
                    shape_str += ", ";
            }
            shape_str += "]";
    ...
    TritonModelInfo parseModelGrpc(const inference::ModelMetadataResponse& model_metadata,
                                   const inference::ModelConfigResponse& model_config);
    TritonModelInfo getModelInfo(
    ...
    std::unique_ptr<SharedMemoryRegion> createSharedMemoryRegion(const std::string& name,
                                                                 size_t size);
    std::unique_ptr<SharedMemoryRegion> createSystemSharedMemoryRegion(const std::string& name,
                                                                       size_t size);
    std::unique_ptr<SharedMemoryRegion> createCudaSharedMemoryRegion(const std::string& name,
                                                                     size_t size);
    void registerInputSharedMemory() override;
    void registerOutputSharedMemory() override;
    void unregisterSharedMemory() override;
    std::vector<Tensor> inferWithSharedMemory(
        const std::vector<std::vector<uint8_t>>& input_data) override;
