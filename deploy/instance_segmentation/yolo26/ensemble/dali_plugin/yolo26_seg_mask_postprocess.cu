...
__device__ float MaskSigmoidDot(const float* coefficients, const float* prototypes, int y,
                                int x) {
    float sum = 0.0F;
...
__global__ void BuildPackedMasks(const float* detections, const float* prototypes,
    ...
    const double top = MaskSigmoidDot(coefficients, prototypes, y0, x0) * (1.0 - wx) +
                       MaskSigmoidDot(coefficients, prototypes, y0, x1) * wx;
    const double bottom = MaskSigmoidDot(coefficients, prototypes, y1, x0) * (1.0 - wx) +
                          MaskSigmoidDot(coefficients, prototypes, y1, x1) * wx;
    const float value = static_cast<float>(top * (1.0 - wy) + bottom * wy);
    masks[output_index] = value > threshold ? 255 : 0;
}

template <typename T>
void ResizeMaskOutput(::dali::Workspace& workspace, int index,
                      const ::dali::TensorShape<>& shape) {
...
class Yolo26SegMaskPostprocess final : public ::dali::Operator<::dali::GPUBackend> {
public:
    explicit Yolo26SegMaskPostprocess(const ::dali::OpSpec& spec)
...
protected:
    ...
    void RunImpl(::dali::Workspace& workspace) override {
        ...
        ResizeMaskOutput<int32_t>(workspace, 0, {1});
        ResizeMaskOutput<int32_t>(workspace, 1, {kMaxDetections, 4});
        ResizeMaskOutput<float>(workspace, 2, {kMaxDetections});
        ResizeMaskOutput<int32_t>(workspace, 3, {kMaxDetections});
        ResizeMaskOutput<int64_t>(workspace, 4, {kMaxDetections + 1});
        ResizeMaskOutput<uint8_t>(workspace, 5, {std::max<int64_t>(1, total_pixels)});
        ...
        copy_to_output(4, mask_offsets.data(), mask_offsets.size() * sizeof(int64_t));
        if (count == 0) {
        ...
        const auto* boxes_device =
            static_cast<const int32_t*>(workspace.Output<::dali::GPUBackend>(1).raw_tensor(0));
        auto* masks_device =
            static_cast<uint8_t*>(workspace.Output<::dali::GPUBackend>(5).raw_mutable_tensor(0));
        ...
        BuildPackedMasks<<<blocks, threads, 0, stream>>>(
            ...
            masks_device, count, mask_threshold_, total_pixels);
...
DALI_REGISTER_OPERATOR(Yolo26SegMaskPostprocess,
                       ::tritonic::dali_plugin::Yolo26SegMaskPostprocess, ::dali::GPU);
DALI_SCHEMA(Yolo26SegMaskPostprocess)
    .DocStr("YOLO26 segmentation packed-mask postprocessing on CUDA")
    .NumInput(3)
    .NumOutput(6)
